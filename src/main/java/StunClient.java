...
public class StunClient {
    ...
    public static void main(String[] args) {
        final String serverHost = "35.215.165.210"; // 中介服务器的IP地址
        final int serverPort = 12345; // 中介服务器的端口号

        try {
            Socket serverSocket = new Socket(serverHost, serverPort);
            System.out.println("Connected to Mediator Server");

            BufferedReader reader = new BufferedReader(new InputStreamReader(serverSocket.getInputStream()));
            String otherClientInfo = reader.readLine();
            System.out.println("Received info of other client: " + otherClientInfo);

            String[] parts = otherClientInfo.split(":");
            String otherClientIp = parts[0];
            int otherClientPort = Integer.parseInt(parts[1]);

            // 与另一个客户端建立P2P连接
            Socket otherClientSocket = new Socket(otherClientIp, otherClientPort);
            System.out.println("Connected to other client: " + otherClientSocket.getRemoteSocketAddress());

            // 启动两个线程分别处理与中介服务器和另一个客户端的通信
            new Thread(new ServerHandler(serverSocket)).start();
            new Thread(new ClientHandler(otherClientSocket)).start();
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void run() {
        try {
            ...
            while ((line = reader.readLine()) != null) {
                System.out.println("Received from Mediator Server: " + line);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
...
class ClientHandler implements Runnable {
    ...
    public ClientHandler(Socket otherClientSocket) {
